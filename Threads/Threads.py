...
def descargarImagenes(urls):
    # Itera sobre cada URL y descarga las imágenes
    for index, url in enumerate(urls, start=1):
        archivo = f'imagen_descargada_{index}.jpg'  # Nombre de archivo único para cada imagen
        try:
            urllib.request.urlretrieve(url, archivo)
            print(f"La imagen {index} ha sido descargada con éxito.")
        except Exception as e:
            print(f"No se pudo descargar la imagen {index}: {e}")

tiempoInicial = time.time()
descargarImagenes(urls)
tiempoFinal = time.time()
TT = tiempoFinal - tiempoInicial
print(TT)#Me dio 8.4 segundos
